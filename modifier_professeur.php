...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['id']) && !empty($_POST['id'])) {
    ...
    $query_select_professeur = "SELECT * FROM utilisateurs WHERE id = ?";
    $stmt = $conn->prepare($query_select_professeur);
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $result_select_professeur = $stmt->get_result();

    if ($result_select_professeur->num_rows > 0) {
        $row_professeur = $result_select_professeur->fetch_assoc();
        ...
        $query_update_professeur = "UPDATE utilisateurs SET nom = ?, prenom = ?, email = ?, genre = ?, adresse = ?, datenaissance = ?, telephone = ? WHERE id = ?";
        $stmt = $conn->prepare($query_update_professeur);
